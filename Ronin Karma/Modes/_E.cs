    ...
    public static class _E
    {
        ...
        static _E()
        {
            ...
            if (enemies.Any(o => o.ChampionName.Equals("Kalista")))
                MiscMenu.Add("Oathsworn", new CheckBox("Block Oathsworn Knockup (Kalista R)", true));
            ...
            foreach (var unit in enemies)
            {
                ...
                foreach (var spell in _E.BlockedSpells[unit.ChampionName])
                {
                    ...
                    MiscMenu.Add(unit.ChampionName + spell.MenuName, new CheckBox(unit.ChampionName + " - " + spell.DisplayName, true));
                }
            }
        ...
        {           
            if (MiscMenu["blockSpellsE"].Cast<CheckBox>().CurrentValue == true || SpellsManager.E.IsReady())
            foreach (var skillshot in Evade.Evade.GetSkillshotsAboutToHit(Player.Instance, (int)(SpellsManager.E.CastDelay + Game.Ping /2f)))
            {           
                ...
                foreach (var spell in spells)
                {
                    ...
                    if (!spell.PassesSpellCondition(skillshot.SpellData.SpellName))
                        continue;
                    if (MiscMenu["blockSpellsE"].Cast<CheckBox>().CurrentValue == true)
                    {
        ...
        public static bool Contains(AIHeroClient unit, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (args.SData.Name.Equals("KalistaRAllyDash") && MiscMenu["Oathsworn"].Cast<CheckBox>().CurrentValue)
            {
                return true;
            }
            var spells = new List<Block>();
            ...
            foreach (var spell in
                spells)
            {
                var item = MiscMenu[name + spell.MenuName];
                if (item == null || !item.Cast<CheckBox>().CurrentValue)
                {
                    continue;
                }
